# UCM – Contenido pendiente de carga

Este documento lista toda la información que debe solicitarse al cliente antes de publicar la web.

## Datos generales de UCM

- [ ] Logo definitivo de UCM (SVG y PNG)
- [ ] Favicon e íconos para manifest (icon-192.png, icon-512.png)
- [ ] Email general de UCM
- [ ] Teléfono general de UCM
- [ ] WhatsApp general de UCM

## Profesionales

- [ ] Listado completo de profesionales con:
  - Nombre y apellido
  - Especialidad
  - Matrícula (opcional)
  - Formación académica
  - Experiencia profesional
  - Áreas de atención
  - Sedes donde atiende
  - Fotografía profesional

- [ ] **Dra. Florencia Calaramo**: número real de WhatsApp del consultorio particular (actualmente hay un número de ejemplo `2216799773` cargado en `src/data/professionals.ts`).

## Servicios

- [ ] Confirmación de servicios disponibles en cada sede
- [ ] Descripción validada de cada servicio por el equipo médico
- [ ] Duración estimada de cada práctica
- [ ] Preparación previa para cada estudio/procedimiento
- [ ] Preguntas frecuentes validadas

## Equipamiento

- [ ] Nombre del mamógrafo
- [ ] Marca y modelo del mamógrafo
- [ ] Descripción técnica validada
- [ ] Nombre del ecógrafo
- [ ] Marca y modelo del ecógrafo
- [ ] Descripción técnica validada
- [ ] Otros equipos a incluir
- [ ] Fotografías del equipamiento
- [ ] Sede donde se encuentra cada equipo

## Sedes

### Instituto Médico Platense
- [ ] Teléfono de contacto
- [ ] Número de WhatsApp para turnos de imágenes
- [ ] Número de WhatsApp para intervencionismo
- [ ] Email de contacto
- [ ] Horarios de atención UCM
- [ ] URL del portal del paciente
- [ ] URL del sistema de turnos online
- [ ] Información de accesibilidad
- [ ] Fotografías de las instalaciones

### Centro Médico de Diagnóstico City Bell
- [ ] Teléfono de contacto
- [ ] Número de WhatsApp para turnos de imágenes
- [ ] Número de WhatsApp para intervencionismo
- [ ] Email de contacto
- [ ] Horarios de atención UCM
- [ ] URL del portal del paciente
- [ ] URL del sistema de turnos online
- [ ] Información de accesibilidad
- [ ] Fotografías de las instalaciones

## Turnos y canales de contacto

- [ ] Número de WhatsApp para mamografías – IMP
- [ ] Número de WhatsApp para mamografías – City Bell
- [ ] Número de WhatsApp para intervencionismo – IMP
- [ ] Número de WhatsApp para intervencionismo – City Bell
- [ ] URL del portal de turnos – IMP
- [ ] URL del portal de turnos – City Bell
- [ ] Canal especial para biopsias y marcaciones (si corresponde)

## Portales del paciente

- [ ] URL Portal del Paciente – Instituto Médico Platense
- [ ] URL Portal del Paciente – Centro Médico City Bell
- [ ] Instrucciones de acceso para pacientes

## Contenido institucional

- [ ] Texto institucional validado (misión, visión, historia)
- [ ] Valores definitivos de la unidad
- [ ] Texto sobre el enfoque multidisciplinario

## Prevención

- [ ] Dato preventivo destacado (con fuente)
- [ ] Contenido validado por el equipo médico sobre:
  - Controles periódicos recomendados
  - Autoobservación mamaria
  - Factores que requieren evaluación
  - Mitos y verdades frecuentes
- [ ] Artículos o material educativo aprobado

## Fotografías

Todas las imágenes placeholder deben ser reemplazadas por:
- [ ] Fotografías de las instalaciones (ambas sedes)
- [ ] Fotografías del equipamiento
- [ ] Fotografías del equipo profesional (individuales)
- [ ] Fotografías de la atención (consultas, estudios)
- [ ] Imágenes para sección de prevención
- [ ] Imágenes para artículos/novedades

## Legal

- [ ] Política de privacidad (texto legal)
- [ ] Términos y condiciones (texto legal)
- [ ] Aviso legal (texto legal)
- [ ] Obras sociales y prepagas aceptadas

## Novedades

- [ ] Artículos o notas reales para la sección de novedades
- [ ] Definición de CMS para gestión de contenidos futura

## SEO

- [ ] Verificación de Google Search Console
- [ ] Configuración de Google Analytics (si corresponde)
- [ ] Imágenes para Open Graph (og:image)
